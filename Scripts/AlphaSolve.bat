...
:vp9Select
ECHO Note: Webm files may appear to have strange chromatic artifacts around edges when viewed in discord embeds while still, windows file explorer, or windows media player, but will appear correctly in many web applications.
...
:gifSelect
ECHO WARNING: Gif files do not support 8-bit transparency, may have less accurate color, and may skip frames due to using 50fps
...
rem Unboosted
%ffmpegPath% -v error -stats -i %1 -filter_complex "[0] split=3 [a][b][c],[a]select=%select1%%ptsAdjust%[blk],[b]select=%select2%%ptsAdjust%[wht],[c]select=%select3%%ptsAdjust%[blk2],[blk]format=gbrp[v1],[wht]format=gbrp[v2],[v1][v2]blend=all_mode=difference,negate,format=yuv420p[map],[map]setpts=0.5*PTS[alphaMap],[alphaMap]split=2[alphaMapUse][out2],[out2]split [ia][ka];[ia] palettegen [pa];[ka]fifo[ma];[ma][pa] paletteuse=dither=none[out3],[alphaMapUse]format=argb,geq=a='255':r='max(max(r(X,Y),g(X,Y)),b(X,Y))':g='max(max(r(X,Y),g(X,Y)),b(X,Y))':b='max(max(r(X,Y),g(X,Y)),b(X,Y))'[alphaMax],[blk2]setpts=0.5*PTS[d],[d][alphaMax]alphamerge,split [i][k];[i] palettegen [p];[k]fifo[m];[m][p] paletteuse=dither=none:alpha_threshold=64" -q:v 0 -r 50 "%~dp1\fullTransparent - %~n1.%outputExt%" -map "[out3]" -q:v 0 -r 50 "%~dp1\alphaMap - %~n1.%outputExt%"
goto endExtSelect
